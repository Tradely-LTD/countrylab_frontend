...
import { Input, Skeleton, StatusBadge } from "../../components/ui";
...
function SampleSearch({ onSelect }: { onSelect: (s: any) => void }) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      api
        .get("/samples", { params: { search, status: "received", limit: 10 } })
        .then((r) => r.data.data),
        ...
        >
          <div>
            ...
            <p className="text-xs text-lab-muted">
              {s.client?.name} · <code className="font-mono">{s.ulid}</code>
            </p>
          </div>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="pt-2 border-t border-lab-border">
        <button
          onClick={onSkip}
          className="text-sm text-lab-muted hover:text-lab-text transition-colors"
        >
